...
@st.cache
def load_LifeData():
    fluorstr = requests.get('https://raw.githubusercontent.com/fdsteffen/Lifefit/master/data/lifetime/Atto550_DNA.txt')
    irfstr = requests.get('https://raw.githubusercontent.com/fdsteffen/Lifefit/master/data/IRF/irf.txt')
...
def load_AnisoData(channels):
    ...
    fluor = {}
    for c in channels:
        fluorstr[c] = requests.get('https://raw.githubusercontent.com/fdsteffen/Lifefit/master/data/anisotropy/{}.txt'.format(c))
...
def get_AnisoFitparams(aniso, model):
    ...
    fit_parameters['p'] = ['{:0.2f} +/- {:0.2f}'.format(aniso.fit_param[i], aniso.fit_param_std[i]) for i,p in enumerate(aniso.param_names[model])]
...
def main():
    ...
    st.markdown('LifeFit analyzes time-correlated single-photon counting (TCSPC) data sets, '
    ...
    st.sidebar.image('https://raw.githubusercontent.com/fdsteffen/Lifefit/master/docs/source/_static/lifefit_logo.png', width=250)

    datatype = st.sidebar.selectbox(label='Type of dataset', options=('Fluorescence Lifetime', 'Fluorescence Anisotropy'))
        ...
        if all([f is not None for f in fluor.values()]) and (irf is not False):
            ...
            with st.spinner('Fitting...'):
                try:
                    ...
                    st.table(fit_parameters)
                    if modelfunc == 'local_global_rotation' or modelfunc == 'hindered_rotation':
                        st.write('Dye populations: **free**: {:0.0f}%    **stacked**: {:0.0f}%'.format(aniso.aniso_fraction[0]*100, aniso.aniso_fraction[1]*100))

                    fig = make_subplots(rows=2, cols=1, row_heights=[0.7, 0.3], vertical_spacing=0.1)
                    xlimits = st.slider('Select time limits (ns) on the x-axis (does not affect the fit)', min_value=float(min(aniso.time)), max_value=float(max(aniso.time)), value=(float(min(aniso.time)), float(max(aniso.time))), format='%0.1f ns')
                    fig.add_trace(go.Scatter(x = aniso.time, y = aniso.r, line=dict(color='rgb(79, 115, 143)', width=1), name='data'), row=1, col=1)
                    fig.add_trace(go.Scatter(x = aniso.time, y = aniso.fit_r, line=dict(color='black', width=1), name='fit'), row=1, col=1)
                    if show_residuals:
                        fig.add_trace(go.Scatter(x = aniso.time, y = aniso.r-aniso.fit_r, line=dict(color='rgb(79, 115, 143)', width=1), name='fit'), row=2, col=1)
                        fig.update_layout(template='none', xaxis2_title='time (ns)', yaxis_title='anisotropy', yaxis2_title='residuals', xaxis1 = dict(range = xlimits), xaxis2 = dict(range = xlimits), yaxis1 = dict(range = (0,0.4)), showlegend=False)
                    else:
